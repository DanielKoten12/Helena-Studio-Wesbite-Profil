      ...
      <div class="relative max-w-7xl mx-auto">
        ...
        <div class="flex items-center justify-center gap-4 mt-8">
          ...
          <button
            ...
            :class="{ 'opacity-50 cursor-not-allowed hover:bg-white hover:text-inherit': !canNavigate }"
          ...
          <div class="flex justify-center gap-2">
            <button
              ...
              :class="[
                (pageIndex - 1) === currentIndex 
                  ? 'bg-[#544771] w-8 h-2' 
                  : 'bg-[#544771] opacity-30 hover:opacity-60 w-2 h-2'
              ]"
              :aria-label="`Go to slide ${pageIndex}`"
            />
          </div>
...
const containerMaxWidth = computed(() => {
  const gap = 24
  return `${(cardWidth.value * cardsPerView.value) + (gap * Math.max(0, cardsPerView.value - 1))}px`
})
...
const canNavigate = computed(() => testimonials.value.length > cardsPerView.value)
...
const handlePrev = () => {
  if (!canNavigate.value || isTransitioning.value) return
...
onMounted(async () => {
  try {
    const data = await dataService.getHomeTestimonials(10)
    testimonials.value = data.map(t => ({
      id: t.id,
      name: t.customer_name,
      rating: t.rating,
      image: t.image_url || 'https://via.placeholder.com/400x300?text=Customer',
